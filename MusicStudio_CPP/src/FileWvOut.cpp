...
*/
void FileWvOut::stereoTick( const StkFloat sampleChannelOne, const StkFloat sampleChannelTwo )
{
#if defined(_STK_DEBUG_)
  if ( !file_.isOpen() ) {
    oStream_ << "FileWvOut::tick(): no file open!";
  ...
  unsigned int nChannels = data_.channels();
  if ( nChannels != 2 ) {
    std::cout << "FileWvOut::stereoTick(): this is not a stereo file!";
    handleError( StkError::WARNING );
    return;
  }
  StkFloat inputOne = sampleChannelOne;
  clipTest( inputOne );
  StkFloat inputTwo = sampleChannelTwo;
  clipTest( inputTwo );
  data_[iData_++] = inputOne;
  data_[iData_++] = inputTwo;
